...
setwd('~/Desktop/kog_mwu/')
# Edit these to match your data file names: 
input=
input="stress_ForMWU.csv" # two columns of comma-separated values: gene id, continuous measure of significance. To perform standard GO enrichment analysis based on Fisher's exact test, use binary measure (0 or 1, i.e., either sgnificant or not).
goAnnotations="Lachancea_kluyveri_cds.emapper.diamond.annotations.emapper.annotations_gene2go.tsv" # two-column, tab-delimited, one line per gene, multiple GO terms separated by semicolon. If you have multiple lines per gene, use nrify_GOtable.pl prior to running this script.
...
input='5FU_ForMWU.csv'

goDivisions = c('CC', 'MF', 'BP')
inputList0 = list.files('./', '_ForMWU.csv')
inputList=inputList0[!grepl('MWU_', inputList0) & !grepl('MF_', inputList0) & !grepl('CC_', inputList0) & !grepl('BP_', inputList0) & !grepl('.tmp', inputList0)]
inputList

for (input in inputList){
  print(paste(input, '...', sep=''))
  for (goDivision in goDivisions){
    print(paste(goDivision, '...', sep=''))
