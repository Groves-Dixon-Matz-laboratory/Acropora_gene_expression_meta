...
setwd('wgcna/go_mwu_module_style')
goDatabase="go.obo"
goAnnotations="amil_zachFullerV2_gos.tsv"
source("gomwu.functions.R")

#set variables to run for each module
mm = read_tsv('../../results_tables/wgcna_moduleMembership.tsv')
mm


scols = c('MMred', 'MMwhite', 'MMgreen4')
for (col in scols){
  out=data.frame(gene=mm$gene,
                 mm=mm[,col])
  outName=paste(col,'goInput.csv', sep='_')
  write_csv(out, outName)
}

inputFiles = paste(scols, 'goInput.csv', sep='_')
...
input='MMred_goInput.csv';COL='red';CUT=0.005;goDivision='BP'
input='MMred_goInput.csv';COL='red';CUT=0.01;goDivision='MF'
input='MMred_goInput.csv';COL='red';CUT=0.05;goDivision='CC'
